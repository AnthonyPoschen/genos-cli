...
- `genos mods draft <serverID> --provider ID [--mod-id ID ...] [--expected-setup ID]` replaces the collection draft (`PUT …/mods/draft`). Omit `--mod-id` for an empty `directModIDs` list.
- `genos mods import <serverID> [--file path.json] [--expected-setup ID]` imports mod-list.json text (`POST …/mods/import`); `--file` or stdin.
- `genos mods draft-apply <serverID> [--expected-setup ID] [--expected-revision N]` applies the collection draft (`POST …/mods/draft/apply`). Autofills setup id and `collection.draft.revision` from GET mods when omitted; fails clearly if no draft revision.
...
- `genos profiles mods list|search|show|credentials|credentials-clear|stage|unstage|discard|apply|draft|import|draft-apply <profileID> …` — same flags as `genos mods`.
...
- `genos public-rcon <serverID>` reveals the public RCON password once (`POST …/public-rcon/credential`). Genos will not show it again. `genos public-rcon <serverID> --rotate --yes` rotates (destructive; `--yes` required).
...
- `genos plan <serverID>` prints read-only plan JSON (`GET …/plan`). **Read-only:** do not invent plan-changes, reactivate, checkout, or portal follow-ups.
