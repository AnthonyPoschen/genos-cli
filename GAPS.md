...
Customer `/api/v1` routes from genos `internal/api/api.go` (admin and Stripe webhook omitted). **Covered in this PR (P1.6)** are marked below.
...
| `…/profiles/{id}/mods…` | — | Profile mods mirrors — immediate follow-up |
...
| `…/profiles/{id}/save-*` | — | Profile save mirrors — immediate follow-up |
...
1. **Profile mods/saves mirrors** — `/profiles/{id}/mods…` and `/profiles/{id}/save-*` (same shapes as server commands).
2. **Billing money flows** — `plan-changes`, `billing/reactivate`, checkout/portal sessions. Do **not** auto-ship spendy flows; if ever added, require explicit human confirmation and clear agent caution labels.
3. **Optional read-only `GET …/plan`** — future caution-labeled slice (no mutations).
4. **Leftovers** — `public-rcon/credential`, SSE/events beyond console, runtime-sessions download, managed files (unreleased), fleet `PUT /server-order`, mods draft/import leftovers.
5. **Device auth** — still blocked on prod until genos #216.
6. Out of scope here: Omarchy, CatalogStatus/sales, Factorio/PZ private product, #212/#64, private genos/controller PRs.
